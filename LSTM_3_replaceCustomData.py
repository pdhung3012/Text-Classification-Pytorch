...
vocab = build_vocab_from_iterator(yield_tokens(all_data), specials=["<unk>"])
vocab.set_default_index(vocab["<unk>"])

lstClasses=[]
for tup in all_data:
    lstClasses.append(tup[1])
lstClasses=set(lstClasses)
num_class = len(lstClasses)
...
train_dataloader = DataLoader(dt_train, batch_size=BATCH_SIZE,
                              collate_fn=collate_batch)
valid_dataloader = DataLoader(dt_valid, batch_size=BATCH_SIZE,
                               collate_fn=collate_batch)
test_dataloader = DataLoader(dt_test, batch_size=BATCH_SIZE,
                              collate_fn=collate_batch)
